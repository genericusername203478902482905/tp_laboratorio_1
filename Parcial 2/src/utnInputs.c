...
int utn_getInt(int *pResultado, char *mensaje, char *mensajeError, int min,
...
int utn_getFloat(float *pResultado, char *mensaje, char *mensajeError,
		float min, float max, int reintentos) {
	int ret = 0;
	float num;
...
int utn_getString(char *input, char mensaje[], char mensajeError[], int min,
		int max, int reintentos) {
	char nombre[51];
	char aux[256];
	int retorno = -1;
...
int utn_charMenu(char *opcion, int max) {
	...
	if (opcion != NULL && max > 0) {
		...
		for (int i = 0; i < max && opcion[i] != '\0'; i++) {
			if ((opcion[i] < 'a' || opcion[i] > 'h')
					&& (opcion[i] < ' ' || opcion[i] > ' ')
					&& (opcion[i] < 'A' || opcion[i] > 'H')) {
				break;
			} else {
				retorno = 0;
			}
